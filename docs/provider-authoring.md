...
The Git provider also accepts a `GitIdentityStrategy` extension point for host-supplied revision attribution. Its `ResolveIdentity` function receives a `RevisionIdentityRequest` with the workspace root and the host of the bound remote location (`TargetHost`, `None` for local paths), and is called for each operation that may create a revision. The result is not cached when a session opens. The host is the same one credential resolution uses, so an application that keeps one account per hub can return the identity that matches where the revision will be published. Returning `Some` supplies the author and committer name and email for that operation. Returning `None` uses the repository's configured `user.name` and `user.email`. If neither a strategy identity nor a complete repository identity is available, the operation fails before mutation with the stable validation code `identity_missing` and a recovery action describing how to configure Git or supply the strategy.
