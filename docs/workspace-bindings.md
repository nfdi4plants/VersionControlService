...
2. Pass that binding to `ProviderResolver.resolve`. An explicit binding selects its registered factory; probes are not consulted.
...
Providers that need mutable indexes, transactions, or recovery data keep them outside the managed workspace. The lakeFS factory requires a host-selected `StateRoot`; every state reference resolves beneath that root and outside every workspace. Missing, corrupt, mismatched, or in-workspace state fails structurally. The provider does not recreate state silently.
