...
/// Service hook used by Git operations to resolve an access token for a remote host.
/// Host applications install the active provider after sign-in; tests and startup code can reset it to the default provider.
...
module RemoteProvisioning =
    ...
    let defaultProvider: Provider = {
        CreateProject =
            fun _ -> promise {
                return Error "No remote provisioning provider is configured. Configure one before publishing this local repository."
            }
    }
